...
const DEFAULTS = {
  ...
  snapshotBlogDir: String(process.env.SNAPSHOT_BLOG_DIR || "").trim(),
...
async function main() {
  ...
  const config = {
    ...
    host: String(args.host || DEFAULTS.host || "").trim(),
    port: String(args.port || DEFAULTS.port || "").trim(),
    ...
    rootAdminPubkey: "",
    blobCacheBaseUrl: String(args.blobCacheBaseUrl || DEFAULTS.blobCacheBaseUrl || "").trim(),
    snapshotBlogDir: String(args.snapshotBlogDir || DEFAULTS.snapshotBlogDir || "").trim(),
    snapshotBlogIndex: String(args.snapshotBlogIndex || DEFAULTS.snapshotBlogIndex || "").trim(),
    snapshotEntitiesPath: String(args.snapshotEntitiesPath || DEFAULTS.snapshotEntitiesPath || "").trim(),
    snapshotMarker: String(args.snapshotMarker || DEFAULTS.snapshotMarker || "").trim(),
  ...
  const envValues = {
    ...
    APP_TAG: config.appTag,
    PROTOCOL_PREFIX: config.protocolPrefix,
    BLOB_CACHE_BASE_URL: config.blobCacheBaseUrl,
    SNAPSHOT_REPO_DIR: toPortablePath(config.repoDir),
    SNAPSHOT_BLOG_DIR: config.snapshotBlogDir,
    SNAPSHOT_BLOG_INDEX: config.snapshotBlogIndex,
    SNAPSHOT_ENTITIES_PATH: config.snapshotEntitiesPath,
    SNAPSHOT_MARKER: config.snapshotMarker,
